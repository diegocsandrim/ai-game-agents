...
function Influence:Create(w, h, window_w, window_h, map)
    ...
    function this:update(map)
        this:ClearInfluence()
        for i,staticLine in ipairs(map.static) do
          this:UpdateInfluence(staticLine)
        end
        
        this:UpdateInfluence(map.dynamic)
    end
            ...
            for y=tiley-1,tiley+1 do
                if (x > 0 and x <= w and y > 0 and y <= h) then
                    ...
                    if(tilex ~= x and tiley ~= y) then
                        ...
                        if(othersInfluence > maxValue and person.team == 2) then
                            maxValue = othersInfluence
                            targetTile.x = x
                            targetTile.y = y
                        end

                        if(othersInfluence < minValue and person.team == 1) then
                            minValue = othersInfluence
                            targetTile.x = x
                            targetTile.y = y
                        end
                    end
